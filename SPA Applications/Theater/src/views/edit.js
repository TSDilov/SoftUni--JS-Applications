...
import { editTheather, getTheaterById } from '../api/data.js';
...
export async function editPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        if (title == '' || date == '' || author == '' || description == '' || imageUrl == '') {
            return alert('All fielda are required!');
        }

        await editTheather(ctx.params.id, { title, date, author, description, imageUrl });
