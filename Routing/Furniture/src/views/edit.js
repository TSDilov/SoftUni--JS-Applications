...
export function editPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        const emptyFields = formData.filter(([k, v]) => k != 'material' && v.trim() == '');
        ...
        try {
            ...
            if (data.make.length < 4 || data.model.length < 4) {
                throw{
                    error: new Error('Model and Make must be at least four symbols!'),
                    errors: {
                        make: true,
                        model: true
                    }
            ...
            if (data.description.length <= 10) {
                throw{
                    error: new Error('Description must be at least ten symbols'),
                    errors: {
                        description: true
                    }
                }
            ...
            if (data.price <= 0) {
                throw{
                    error: new Error('Price must be positive number'),
                    errors: {
                        price: true
                    }
                }
